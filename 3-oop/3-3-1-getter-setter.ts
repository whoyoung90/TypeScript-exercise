...
/* constructor에 접근제어자(public, private)를 설정해두면 바로 멤버변수로 설정이 된다! */
{
  class User4 {
    get fullName(): string {
      return `${this.firstName} ${this.lastName}`;
...

{
  ...
  // user.internalAge = 33; // Property 'internalAge' is private and only accessible within class 'User'.
...
/**
 * @description 정리
 * 내부 상태값은 외부에서 직접 조작하지 못하고
 * 함수(fillCoffeeBeans)나 setter를 통해 변경해야 한다 !!
 */
